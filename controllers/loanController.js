...
exports.applyForLoan = async (req, res) => {
  try {
    // The member may belong to several cooperatives; the frontend sends the
    // cooperative they currently have selected.
    ...
    if (!ctx) {
      return res.status(400).json({ message: 'You must be an active member of a cooperative before applying for a loan' });
    }

    const customerId = ctx.customerId;
    const cooperativeId = ctx.cooperativeId;
